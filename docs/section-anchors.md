...
2. failing that (no title), the **label part of the structural position** —
   `papers` of `"4.4.papers"`, see [structural-position](structural-position.md)
   — except for the meaningless default `"5.5.default"`;
3. failing that, the generic `view`.
...
through to rule 2 or 3 above — such sections are anchored by their structural
...
- `addSectionAnchors()` appends a faint `#` handle to each section title
  (`.paneltitlerow > h4`, `.view-header-titlerow > h3`, …). It is a real link to
